...
## Captions / licensing
- Caption: YARL slide `title` = `Artist · Title, year` (+ license for CC), with graceful omission. YARL slide `description` = the optional **subject `note`** — a short locator ("Ajax — the warrior at right (inscribed AIANTOS)") added only to multi-figure artworks in figure-subject entries where the subject isn't obvious; written by viewing each image (vision pass), conservative (blank when single-figure/obvious/uncertain). ~165 of 514 images have a `note`. **English titles only** — foreign/messy/catalogue titles were stripped. **All** titles (harvested + originals) were standardised: translated to English, museum/inventory/accession noise removed, truncations/"artist-as-title"/book-scan prefixes fixed, Title Case; `artist` fields scrubbed of Commons uploader/photographer handles (kept real painters + vase-painter conventions; blanked anonymous ancient works). Re-standardise from the `source` filename (ground truth) — the stored `title` was sometimes lossy/truncated.
